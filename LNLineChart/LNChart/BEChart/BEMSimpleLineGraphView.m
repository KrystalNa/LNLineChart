...
@interface BEMTouchView : UIView
@property (nonatomic,assign) NSInteger index;/**< 同点击的BEMCircle的tag*/
@end
@implementation BEMTouchView
@end
...
//        circleDot.center = CGPointMake(self.frame.size.width/2, self.frame.size.height/2);
        ...
        if ([self.dataSource respondsToSelector:@selector(lineGraph:valueForPointAtIndex:)]) {
            dotValue = [self.dataSource lineGraph:self valueForPointAtIndex:0];
...
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
            dotValue = [self.delegate valueForIndex:0];
#pragma clang diagnostic pop
        ...
        } else [NSException raise:@"lineGraph:valueForPointAtIndex: protocol method is not implemented in the data source. Throwing exception here before the system throws a CALayerInvalidGeometry Exception." format:@"Value for point %f at index %lu is invalid. CALayer position may contain NaN: [0 nan]", dotValue, (unsigned long)0];
#else
        dotValue = (int)(arc4random() % 10000);
#endif
        
        CGFloat positionOnYAxis = [self yPositionForDotValue:dotValue];
        circleDot.tag = DotFirstTag100;
        circleDot.center = CGPointMake(0, positionOnYAxis);
        circleDot.Pointcolor = self.colorPoint;
        circleDot.isLastes = YES;
        circleDot.circleType = self.circleType;
        circleDot.alpha = 1;
        circleDot.absoluteValue = dotValue;
        [self addSubview:circleDot];
        
        //添加一个放大的view获取点击事件
        BEMTouchView *bigCircleDot = [[BEMTouchView alloc] initWithFrame:CGRectMake(0, 0, self.sizePoint + 25*MLPercent, self.sizePoint + 25*MLPercent)];
        bigCircleDot.center = CGPointMake(0, positionOnYAxis);
        bigCircleDot.index = DotFirstTag100;
        [bigCircleDot addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(dotsAction:)]];
        [self addSubview:bigCircleDot];
...
#pragma clang diagnostic pop
            ...
            if (dotValue != BEMNullGraphValue) {
                ...
                circleDot.absoluteValue = dotValue;
                circleDot.Pointcolor = self.colorPoint;
                if (i==self.curSelectIndex&&self.isHaveSelect) {
                    circleDot.isLastes = YES;
                }else{
                    circleDot.isLastes = NO;
                }
                circleDot.circleType = self.circleType;
                [self addSubview:circleDot];
                ...
                //添加一个放大的view获取点击事件
                BEMTouchView *bigCircleDot = [[BEMTouchView alloc] initWithFrame:CGRectMake(0, 0, self.sizePoint + 25*MLPercent, self.sizePoint + 25*MLPercent)];
                bigCircleDot.center = CGPointMake(positionOnXAxis, positionOnYAxis);
                bigCircleDot.index = i + DotFirstTag100;
                [bigCircleDot addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(dotsAction:)]];
                [self addSubview:bigCircleDot];
...
- (void)dotsAction:(UITapGestureRecognizer *)sender {
    for (UIView *subview in [self subviews]) {
        if ([subview isKindOfClass:[BEMCircle class]]){
            BEMCircle *subviewTemp = (BEMCircle *)subview;
            if (subviewTemp.isLastes) subviewTemp.isLastes = NO;
        }
    }
    
    BEMTouchView *touchView = (BEMTouchView *)sender.view;
    NSInteger circleTag = touchView.index;
    BEMCircle *circleView = (BEMCircle *)[self viewWithTag:circleTag];
    circleView.isLastes = YES;
    
    if ([self.delegate respondsToSelector:@selector(lineGraph:didTouchDotWithIndex:)]) {
        [self.delegate lineGraph:self didTouchDotWithIndex:(circleTag - DotFirstTag100)];
    }
}

- (void)setCurSelectIndex:(NSInteger)curSelectIndex{
